...
def generate_report(request, elements, title, name='report') -> HttpResponse:
    ...
    def primera_pagina(canvas, doc):
        ...
        footer = Paragraph(f'<p>Generated by the user {request.user.get_full_name()} using CashFlow. </p>')
        ...
        page_number_text = "CashFlow - Page %d" % (doc.page)
    ...
    if year and month:
        ...
        for i in range(1, num_days + 1):
            ...
            sums['avg_balance_mc'] += convert_all_transactions_amounts_to_main_currency_precisely(
                [{'amount': current_balance, 'from_account__currency': account.currency.pk,  'date': day, 'exchange_rate': None}], request.user.main_currency.currency.pk
            )
...
def generate_yearly_transactions_report_all_accounts(request, year) -> HttpResponse:
    ...
    year = int(year)
    table_accounts = [['Month', 'Start Balance', 'Ext. In', 'Int. In', 'Ext. Out', 'Int. Out', 'Exchange Diff.', 'Final Balance', '% Variation']]
    ...
    for month in months:
        ...
        for account in Account.objects.filter(owner=request.user, visible=True):
            start_balance += convert_all_transactions_amounts_to_main_currency_precisely(
                [{'amount': HistoricBalance.objects.filter(
                    ...
                    month=month - 1 if month > 1 else 12,
                    account=account
                ).exists() else 0, 'from_account__currency': account.currency.pk, 'exchange_rate': None, 'date': datetime.date(year, month, 1)}], main_currency.pk
            )
            final_balance += convert_all_transactions_amounts_to_main_currency_precisely(
                [{'amount': HistoricBalance.objects.filter(
                    year=year,
                    month=month,
                    account=account
                    ...
                    month=month,
                    account=account
                ).exists() else 0, 'from_account__currency': account.currency.pk, 'exchange_rate': None, 'date': datetime.date(year, month, calendar.monthrange(year, month)[1])}], main_currency.pk
            )
            for transaction in Transaction.objects.filter(
                date__year=year,
                date__month=month,
                ...
                if transaction.transaction_type == '+':
                    if not transaction.internal:
                        in_ext += converted_amount
                        sums['in_ex'] += converted_amount
                    else:
                        in_int += converted_amount
                        sums['in_int'] += converted_amount
                else:
                    if not transaction.internal:
                        out_ext += converted_amount
                        sums['out_ex'] += converted_amount
                    else:
                        out_int += converted_amount
                        sums['out_int'] += converted_amount

                    if transaction.tag:
                        if transaction.tag.name not in tags:
                            tags[transaction.tag.name] = 0
        ...
        for transaction in Transaction.objects.filter(
            ...
            if transaction.transaction_type == '+':
                ...
                in_ext += transaction.amount
                sums['in_ex'] += transaction.amount
                sums['exchange_diff'] += transaction.amount
            else:
                out_ext += transaction.amount
                sums['out_ex'] += transaction.amount
                exchange_diff -= transaction.amount
                sums['exchange_diff'] -= transaction.amount
        ...
        table_accounts.append([
            ...
            f'{exchange_diff:,.2f}',
            f'{final_balance:,.2f}',
            f'{((final_balance - start_balance) / start_balance) * 100:,.2f}%' if start_balance != 0 else '-',
        ])
    ...
    return generate_report(
        request,
        title=f'Yearly Summary Report for All Accounts - {year}',
        elements=[
            Paragraph(f'All amounts are represented in {main_currency}', style=ParagraphStyle('normal', fontSize=10)),
            Table(
                table_accounts,
            ...
            Table(
                [
                    ['Tag', 'Total Expense'],
                    *([tag, f'{values:,.2f}'] for tag, values in tags.items())
                ],
                style=[
                    ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
                    ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
            ...
            Table(
                [
                    ['Concept', 'Value'],
                    ['Total Start Balance', f'{first_start_balance:,.2f}'],
                    ['Total Final Balance', f'{final_balance:,.2f}'],
                    ['Total Ext. Expenses', f'{sums["out_ex"]:,.2f}'],
                    ['Total Ext. Income', f'{sums["in_ex"]:,.2f}'],
                    ['Total Int. Expenses', f'{sums["out_int"]:,.2f}'],
                    ['Total Int. Income', f'{sums["in_int"]:,.2f}'],
                    ['Total Exchange Differences', f'{sums["exchange_diff"]:,.2f}'],
                    ['Total Cash Flow', f'{sums["cash_flow"]:,.2f}'],
                    ['% of Variation (Start vs Final Balances)', f'{sums["final_variation"]:,.2f}%'],
                ],
                style=[
                    ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
                    ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
